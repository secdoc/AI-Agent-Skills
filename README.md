...
- **Detailed architecture is mandatory.** Every threat model includes one audit-ready physical and logical network architecture view showing all in-scope zones and trust boundaries, physical and virtual dependencies, routing and enforcement points, significant labeled flows, management paths, external dependencies, failure domains, implementation state, and known control gaps. Focused attack-path, identity, data-flow, recovery, or service-cluster diagrams supplement this view when useful, but never replace it.
